 ...
 * remove-dark-mode.js
 *
 * Removes dark mode functionality from the Beginner-Astro-LESS template.
...
async function main() {
  ...
  console.log('  - Move DarkModeToggle.astro, dark.less, moon.svg, and sun.svg to scripts/deleted/');
  console.log('  - Remove the inline dark mode script from BaseLayout.astro');
  console.log('  - Remove the DarkModeToggle import and usage from Header.astro');
  console.log('  - Remove the dark.less import from BaseLayout.astro');
  console.log('  - Remove dark mode toggle styles from root.less');
  console.log('  - Remove body.dark-mode CSS blocks from all components and pages');
  ...
  updateFile('src/layouts/BaseLayout.astro', [
    ...
    },
    {
      ...
      pattern: /\n*<script is:inline>\s*\n\s*\/\/\s*Script to handle dark mode[\s\S]*?<\/script>\s*/g,
      replacement: '\n',
    },
  ]);

  // --- 3. Update Header.astro ---
  console.log('\n--- Updating Header.astro ---');
  updateFile('src/components/Header.astro', [
    {
      name: 'Remove DarkModeToggle import',
      pattern: /import\s+DarkModeToggle\s+from\s+["']\.\/DarkModeToggle\.astro["'];\n?/g,
      replacement: '',
    },
    {
      name: 'Remove DarkModeToggle usage and comment',
      pattern: /\s*<!--Dark Mode toggle[\s\S]*?-->\s*\n?\s*<DarkModeToggle\s*\/>/gi,
      replacement: '',
    },
  ]);

  // --- 4. Clean dark mode styles from all files ---
  console.log('\n--- Cleaning Dark Mode Styles ---');

  // Global LESS file
  cleanDarkStyles('src/styles/root.less');

  // Components with embedded <style lang="less">
  cleanDarkStyles('src/components/Header.astro');
  cleanDarkStyles('src/components/Footer.astro');
  cleanDarkStyles('src/components/CTA.astro');

  // All pages with embedded <style lang="less">
  ...
  if (fs.existsSync(pagesDir)) {
    ...
    for (const file of pageFiles) {
      cleanDarkStyles(path.join('src/pages', file));
    }
  }
